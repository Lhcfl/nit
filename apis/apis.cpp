 ...
 * @return
 * 返回一个包含目录中所有文件名的字符串向量。如果目录不存在或者无法访问，则可能返回一个空向量。
 */
std::vector<std::string>
UsefulApi::listFilesInDirectory(const std::string &directoryPath) {
 ...
 * @return
 * 返回true表示文件读取成功；返回false表示文件读取失败（例如，文件不存在或无法打开）。
 */
 ...
 */
std::string UsefulApi::hash(const std::string &str) { return SHA1()(str); }